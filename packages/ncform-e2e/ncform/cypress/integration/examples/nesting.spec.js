...
context('Nesting', () => {
  ...
  it('Object nested object', () => {
    ...
    cy.get(`[data-cy=${id}]`).within(() => {
      ...
      cy.get('@name').next().find('input').eq(0).should('have.value', 'daniel')
      cy.get('@name').next().find('input').eq(1).should('have.value', 'xiao')
    })
  })
  ...
  it('Object nested array and table', () => {
    ...
    cy.get(`[data-cy=${id}]`).within(() => {
      ...
      cy.get('@users1').next().find('input').should('have.value', 'daniel');
      ...
      cy.get('@users2').next().find('input').should('have.value', 'sarah');
    })
  })
  ...
  it('Arrays and table items are objects', () => {
    ...
    cy.get(`[data-cy=${id}]`).within(() => {
      ...
      cy.get('@companies1Item').next().next().find('input').should('have.value', 'daniel');
      ...
      cy.get('@companies2').next().find('input').should('have.value', 'sarah');
    })
  })
  ...
  it('Arrays and table items are array and table', () => {
    ...
    cy.get(`[data-cy=${id}]`).within(() => {
      ...
      cy.get('@users').next().find('input').should('have.value', 'daniel');
      ...
      cy.get('@users').next().find('input').its('length').should('equal', 1);
      cy.get('@users').next().find('input').should('have.value', 'sarah');
