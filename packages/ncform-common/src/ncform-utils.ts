 ...
 * @param {*} formSchema
 */
export function perfectFormSchema(formSchema) {
  let returnSchema;
  if (typeof formSchema === "string") {
    try {
      returnSchema = JSON.parse(formSchema);
    } catch {
      throw new Error("fromSchema must be a valid json format");
    }
  ...
  function addDefField(fieldName, fieldSchema) {
    ...
    const fullFields = {
      ...
      // value: getDefVal(fieldSchema.type),
      ...
      ui: {
        ...
        showLabel: true,
        showLegend: (isNormalObjSchema(fieldSchema) || isNormalArrSchema(fieldSchema)) ? true : false,
        ...
        widgetConfig: {
          type: _get(fieldSchema, "type", 'string'),
 ...
 * @param {*} formSchema
 */
export function getModelFromSchema(formSchema, fieldName = "$root") {
  let model: any | any[]  = {};
  ...
  if (formSchema.type === "object") {
    if (formSchema.properties) {
      Object.keys(formSchema.properties).forEach(key => {
        if (
          isNormalObjSchema(formSchema.properties[key]) ||
          isNormalArrSchema(formSchema.properties[key])
        ) {
          model[key] = getModelFromSchema(
            formSchema.properties[key],
            key
          );
        } else {
          if (formSchema.properties[key].type.toUpperCase() !== formSchema.properties[key].type) { // 大写的类型忽略掉
            model[key] = priorityGetValue(
              "basic",
              formSchema.properties[key].value,
              smartAnalyze(formSchema.properties[key].default),
              getDefVal(formSchema.properties[key].type)
            );
          }
        }
      });
    }
    model = priorityGetValue(
      "object",
      model,
      formSchema.value,
      smartAnalyze(formSchema.default)
    );
  } else if (formSchema.type === "array") {
    model = [];

    if (
      isNormalArrSchema(formSchema.items) ||
      isNormalObjSchema(formSchema.items)
    ) {
      model.push(getModelFromSchema(formSchema.items, fieldName));
    } else {
      model.push(
        priorityGetValue(
          "basic",
          formSchema.items.value,
          smartAnalyze(formSchema.items.default),
          getDefVal(formSchema.items.type)
        )
      );
    }

    // model = priorityGetValue('array', formSchema.value, model, formSchema.default);
    model = priorityGetValue(
      "array",
      formSchema.value,
      smartAnalyze(formSchema.default),
    ...
    model = model.map(item => {
      if (item.__dataSchema) {
        // 特殊处理
        return getModelFromSchema(item.__dataSchema, "");
      }
      return item;
    });
  } else {
    ...
    if (isNormalObjSchema(schema)) {
      ...
      schema.value = !parentPath
        ...
        : priorityGetValue(
            ...
            schema.value,
            smartAnalyze(schema.default),
            getDefVal(schema.type)
          );
    }
  }
 ...
 * @param {*} schema
 */
export function isNormalObjSchema(schema) {
  return (
    schema.type === "object" &&
    !!schema.properties &&
 ...
 * @param {*} schema
 */
export function isNormalArrSchema(schema) {
  return (
    schema.type === "array" &&
    !!schema.items &&
 ...
 * @param {*} path
 */
export function getSchemaByPath(fromSchema, pathParam, idxChain) {
  let path = pathParam;
  if (idxChain) {
    // 处理[i]
...
export function smartAnalyzeVal(
  val,
  { idxChain = "", data = { rootData: {}, constData: {}, selfData: {}, tempData: {} } }: AnalyzeValData = {}
) {
  return smartAnalyze(val, {
    idxChain,
    data: [
      {
        symbol: "$root",
 ...
 */
export function smartAnalyze(val, { idxChain = "", data = [], expPrefix = "dx:" } = {}, __get = _get, __map = _map) {
 ...
 * @param {*} dataType
 */
export function getDefVal(dataType) {
  let defVal: any = "";
  switch (dataType) {
    case "string":
      defVal = "";
 ...
 * @param {*} val
 */
export function notEmptyVal(val) {
 ...
 */
export function priorityGetValue(dataType, ...vals) {
  let result;
  switch (dataType) {
    case "object":
 ...
 * @param {*} num
 */
export function genRandomId(num) {
  if (!num) num = 5;
  if (num > 10) num = 10;
  return Math.random()
    .toString(36)
