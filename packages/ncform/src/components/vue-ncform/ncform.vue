...
export default {
  ...
  created () {
    ...
    function handleSchema () {
      const dataFormSchema = ncformUtils.perfectFormSchema(vm.formSchema)
      ...
      ncformUtils.setValueToSchema(
        vm.modelValue || dataFormSchema.value,
        dataFormSchema,
        true
      )
      ...
      vm.$nextTick(() => {
        const rootBorderElement = vm.$el.querySelector('[class$=-layout]')
    ...
    ncformValidate () {
      ...
      return new Promise((resolve, reject) => {
        Promise.all(this.$data.validateArray).then(data => {
          data.forEach(item => {
            _get(this.$data, item.__path).__validationResult = JSON.parse(JSON.stringify(item.result))
          })
          resolve({
